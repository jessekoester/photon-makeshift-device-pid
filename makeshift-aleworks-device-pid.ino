...
void setup() {
    ...
    Particle.variable("temperature", tempInfo, STRING);
    Particle.variable("point", pointInfo, STRING);
    Particle.variable("pwm", pwmInfo, STRING);
    
    // SparkCloud: functions
    Particle.function("setPoint", setPointTemperature);
...
void loop() {
    ...
    if (millis() - lastEventTimestamp >= 1000) {
        ...
        // SparkCore: publish
        Particle.publish("tempInfo", tempInfo);
        Particle.publish("pointInfo", pointInfo);
        Particle.publish("pwmInfo", pwmInfo);
